...
class ChatStorage {
    ...
    func setTotalUnreadMessageCount(completion: @escaping(_ totalNum: Int) -> Void) {
        
        let dialogIDs: NSMutableSet = []
        
        for dia:QBChatDialog in dialogs {
            dialogIDs.add(dia.id ?? "")
        }
        
        QBRequest.totalUnreadMessageCountForDialogs(withIDs: dialogIDs as! Set<String>, successBlock: { ( response, totalUnreadCount, dialogsDictionary) in
