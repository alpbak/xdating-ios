...
class ChatNewViewController: UIViewController {
    ...
    //MARK: - Life Cycle
        override func viewDidLoad() {
            ...
            print("ChatNewViewController - didLoad")
...
//                    }
                    ...
                    guard let selectAssetsVC = storyboard.instantiateViewController(withIdentifier: "SelectAssetsVC") as? SelectAssetsVC else{ return }

                    
                    
                    
                    print("showAllAssets 11111")
            ...
            if let pickerController = pickerController, sourceType == .camera {
                print("showPickerController - CAMERA")
                ...
                print("showPickerController - PHOTO LIBRARY")
                switch PHPhotoLibrary.authorizationStatus() {
                case .authorized:
                    print("showPickerController - AUTHORIZED")
                    showAllAssets()
                case .notDetermined:
                ...
                case .denied, .restricted:
                    accessDenied(sourceType)
                @unknown default:
                    print("showPickerController - UNKNOWN PERMISSON")
                }
            }
        }
        ...
        private func sendMessage(message: QBChatMessage) {
            ...
            chatManager.send(message, to: dialog) { [weak self] (error) in
                if let error = error {
                    debugPrint("[ChatNewViewController] sendMessage error: \(error.localizedDescription)")
                    return
                }
                self?.stopTyping()
    ...
    extension ChatNewViewController: UIScrollViewDelegate {
    ...
    extension ChatNewViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    extension ChatNewViewController: ChatDataSourceDelegate {
    ...
    extension ChatNewViewController: InputToolbarDelegate {
    ...
    extension ChatNewViewController: UICollectionViewDelegate {
    ...
    extension ChatNewViewController: ChatCollectionViewDataSource {
    ...
    extension ChatNewViewController: ChatCollectionViewDelegateFlowLayout {
        func collectionView(_ collectionView: UICollectionView,
    ...
    extension ChatNewViewController: UITextViewDelegate {
    ...
    extension ChatNewViewController: ChatCellDelegate {
    ...
    //MARK: - QBChatDelegate
    extension ChatNewViewController: QBChatDelegate {
    ...
    extension ChatNewViewController: AttachmentBarDelegate {
    ...
    extension ChatNewViewController: UIPopoverPresentationControllerDelegate {
